...
func (h *ProfileHandler) GetProfile(c *fiber.Ctx) error {
	...
	err := h.DB.QueryRow(c.Context(), `
		SELECT id, full_name, phone_number, address, avatar_url, role, created_at
		FROM profiles WHERE id = $1`, id,
	).Scan(&p.ID, &p.FullName, &p.Phone, &p.Address, &p.AvatarURL, &p.Role, &p.CreatedAt)
...
func (h *ProfileHandler) UpdateProfile(c *fiber.Ctx) error {
	...
	err := h.DB.QueryRow(c.Context(), `
		...
		WHERE id = $5
		RETURNING id, full_name, phone_number, address, avatar_url, role, created_at`,
